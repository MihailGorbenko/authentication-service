...
Public API endpoints:
    ...
    -  Route: /register
        ...
        If the user with the provided email is already registered, the route will respond with an HTTP 400 status code and an error message "User already registred", and 'predicate' key with 'EXIST' value.

        If the request body is not valid, the route will respond with an HTTP 400 status code and an error message,'predicate' key with 'INCORRECT' value and 'errors' key with array of errors.
    ...
    - Route: /refreshToken
        ...
        If the refresh token is missing or not valid, the route will respond with an HTTP 400 status code and an error message "Refresh token missing" and 'predicate' key with 'MISSING_TOKEN' value, or "Refresh token not exists or already deprecated" and 'predicate' key with 'BAD_TOKEN' value respectively.
    ...
    - Route: /checkEmail
        ...
        If the user email incorrect or missing, the route will respond with an HTTP 400 status code and a JSON object containing the error message and a 'predicate' key with 'INCORRECT' value.
    ...
    - Route: /resetPassword
        ...
        If the user is not authenticated, the route will respond with an HTTP 401 status code and an error message "User not registred", and 'predicate' key with 'NOT_EXIST' value.
